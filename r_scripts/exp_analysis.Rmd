...
fish_amount = 100000000, thread_num = 128 and schedule = static as it is limited as the max number of thread in a Setonix node
...
fish_amount = 75000000, thread_num = 128 and schedule = static as it is limited as the max number of thread in a Setonix node

```{r fig.width=12, fig.height=8}
df <- subset(exp_data, fish_amount == 7500000 & thread_num == 128)
df[, "process_num"] <- as.factor(df[, "process_num"])
